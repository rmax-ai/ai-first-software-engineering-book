...
- `git --no-pager show HEAD^:state/migration_iterations/iter_081/01-task.md | rg -nF '\`uv run python state/copilot_sdk_smoke_test.py --mode fallback-error\`'` confirms pre-edit escaped wording now uses the normalized command.
