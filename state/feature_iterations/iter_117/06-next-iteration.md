...
## Why it is next
After adjacency hardening, uniqueness hardening for the new mode is the smallest deterministic follow-up to prevent duplicate registration drift.

## Concrete acceptance criteria
1. Add one mode in `state/copilot_sdk_smoke_test.py` asserting `...-uniqueness-adjacency-guard` appears exactly once in `TRACE_SUMMARY_MODE_SPECS`.
...
- `state/feature_iterations/iter_118/*.md`
