...
1. Add `run_usage_examples_duplicate_count_wrapper_helper_uniqueness_order_adjacency_guard_mode()` beside existing duplicate-count wrapper helper hardening guard modes.
2. Build the check from `TRACE_SUMMARY_MODE_SPECS` names: assert both target modes exist, then assert uniqueness-order guard index is exactly one after uniqueness-adjacency guard index.
