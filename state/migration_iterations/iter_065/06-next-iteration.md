...
Normalize the snippet in `state/migration_iterations/iter_055/06-next-iteration.md` to use `uv run python state/copilot_sdk_smoke_test.py --mode fallback-error` consistently.
...
- `rg -n '\`uv run python state/copilot_sdk_smoke_test.py --mode fallback-error\`' state/migration_iterations/iter_055/06-next-iteration.md` finds the legacy snippet before edit.
