...
1. Reuse `_run_usage_examples_duplicate_count_mode_coverage_guard(...)` in exactly two additional wrappers in `state/copilot_sdk_smoke_test.py`.
2. Limit edits to:
...
3. Preserve wrapper-specific PASS strings as-is by passing existing text through helper arguments.
4. Run the two required smoke modes from acceptance criteria.
5. Record execution, validation evidence, risks, one next task, and summary in iter_081 artifacts.
