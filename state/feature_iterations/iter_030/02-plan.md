...
1. Add `run_usage_examples_duplicate_count_mode_coverage_guard_mode` in `state/copilot_sdk_smoke_test.py`.
2. Reuse `_build_parser(...)` and `_usage_doc_lines(...)` helpers to assert the target mode appears in parser choices and generated usage mode names.
...
5. Write all `iter_030` artifacts with execution, validation, risk, and handoff details.
