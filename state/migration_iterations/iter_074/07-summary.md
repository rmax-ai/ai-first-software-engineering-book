...
A single acceptance-criteria line in `state/migration_iterations/iter_072/01-task.md` was normalized to remove bare `python` snippet wording.
...
Targeted validation used literal `rg -nF` checks and a scoped `git --no-pager diff HEAD~1..HEAD` on the edited file.
...
All seven required markdown artifacts were written under `state/migration_iterations/iter_074/`.
